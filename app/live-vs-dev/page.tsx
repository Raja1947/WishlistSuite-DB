...
export default async function LiveVsDevPage() {
  const now = new Date();
  ...
  const [rawStores, rawConvStats] = await Promise.all([
    prisma.$queryRaw<RawRow[]>`
      SELECT
        si.shop, si.name, si.url, si.country, si."currencyCode", si.email, si."planDisplayName", si."partnerDevelopment",
  ...
  return <LiveVsDevView stores={stores} />;
}
